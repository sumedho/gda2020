...
def geo_to_grid(latitude, longitude, central_meridian, proj):
    ...
    A = (a/(1+n)) * (1+(1.0/4.0) * n2 + (1.0/64) * n4 + (1.0/256) * n6 +
                     (25.0/16384) * n8)

    # Calculate conformal latitude (eq 22, 23)
    sigma = math.sinh(e*math.atanh((e * math.tan(rlat))/
    ...
    a2 = (1.0/2) * n - (2.0/3) * n2 + (5.0/16) * n3 + (41.0/180) * n4 -\
         (127.0/288) * n5 + (7891.0/37800) * n6 + (72161.0/387072) * n7 -\
         (18975107.0/50803200) * n8

    a4 = (13.0/48) * n2 - (3.0/5) * n3 + (557.0/1440) * n4 + (281.0/630) * n5\
         - (1983433.0/1935360) * n6 + (13769.0/28800) * n7 +\
        (148003883.0/174182400) * n8

    a6 = (61.0/240) * n3 - (103.0/140) * n4 + (15061.0/26880) * n5 +\
         (167603.0/181440) * n6 - (67102379.0/29030400) * n7 +\
         (79682431.0/79833600) * n8

    a8 = (49561.0/161280) * n4 - (179.0/168) * n5 + (6601661.0/7257600) * n6 +\
         (97445.0/49896) * n7 - (40176129013.0/7664025600) * n8

    a10 = (34729.0/80640) * n5-(3418889.0/1995840) * n6 +\
          (14644087.0/9123840) * n7+(2605413599.0/622702080) * n8

    a12 = (212378941.0/319334400) * n6 - (30705481.0/10378368) * n7 +\
    ...
    v = a * math.asinh(math.sin(w) /
        (math.sqrt(conformal_lat * conformal_lat +
    ...
    grid_conv = math.atan(q/p) + math.atan((conformal_lat * math.tan(w))/
                math.sqrt(1 + conformal_lat * conformal_lat)) * 180/math.pi

    # Calculate transverse Mercator coordinates (eq 29, 30)
    X = A*((v / a) + a2 * x1 + a4 * x2 + a6 * x3 + a8 * x4 + a10 * x5 +
           a12 * x6 + a14 * x7 + a16 * x8)

    Y = A*((u / a) + a2 * y1 + a4 * y2 + a6 * y3 + a8 * y4 + a10 * y5 +
...
def grid_to_geo(easting, northing, central_meridian, projection):
    ...
    b2 = (-1.0/2) * n + (2.0/3) * n2 - (37.0/96) * n3 + (1.0/360) * n4 +\
        (81.0/512) * n5 - (96199.0/604800) * n6 + (5406467.0/38707200) * n7 -\
        (7944359.0/67737600) * n8

    b4 = (-1.0/48) * n2 - (1.0/15) * n3 + (437.0/1440) * n4 - (46.0/105) * n5\
        + (1118711.0/3870720) * n6 - (51841.0/1209600) * n7 -\
        (24749483.0/348364800) * n8

    b6 = (-17.0/480) * n3 + (37.0/840) * n4 + (209.0/4480) * n5 -\
        (5569.0/90720) * n6 - (9261899.0/58060800) * n7 +\
        (6457463.0/17740800) * n8

    b8 = (-4397.0/161280) * n4 + (11.0/504) * n5 + (830251.0/7257600) * n6 -\
         (466511.0/2494800) * n7 - (324154477.0/7664025600) * n8

    b10 = (-4583.0/161280) * n5 + (108847.0/3991680) * n6 +\
          (8005831.0/63866880) * n7 - (22894433.0/124540416) * n8

    b12 = (-20648693.0/638668800) * n6 + (16363163.0/518918400) * n7 +\
    ...
    nn = (X/A) + b2 * x1 + b4 * x2 + b6 * x3 + b8 * x4 + b10 * x5 + b12 * x6 +\
        b14 * x7 + b16 * x8

    # gauss-schreiber ratios (eq 43)
    ee = (Y/A) + b2 * y1 + b4 * y2 + b6 * y3 + b8 * y4 + b10 * y5 + b12 * y6 +\
        b14 * y7 + b16 * y8

    # (eq 44)
    t_dash = math.sin(ee)/(math.sqrt(math.sinh(nn) * math.sinh(nn) +
                                     math.cos(ee) * math.cos(ee)))
    ...
    for i in range(0, 5):
        ...
        # (eq 49)
        ft_dash = (math.sqrt(1 + sigma**2)*math.sqrt(1 + t**2) - sigma*t) * \
                  ((1-e2)*math.sqrt(1+t**2))/(1+(1+e2)*t*t)
        # (eq 47)
        t = t - (ft/ft_dash)
    ...
    calculated_longitude = np.degrees(np.radians(central_meridian) + omega)
...
def geodetic_to_cartesian(latitude, longitude, h, projection):
    ...
    # radius of curvature at a point on ellipsoid with respect to prime
    # vertical through that point (eq 11)
...
def cartesian_to_geodetic(X, Y, Z, projection):
    ...
    # radius of curvature at a point on ellipsoid with respect to meridian(eq4)
    p = math.sqrt(X**2 + Y**2)
    # (eq 6)
    r = math.sqrt(p**2 + Z**2)
    # parametric latitude (eq 5)
    u = math.atan((Z/p)*(subf + (e2 * a/r)))

    # (eq 1)
    longitude_radians = math.atan2(Y, X)
    ...
    h = part1 + part2 - part3
    latitude = dec2dms(math.degrees(latitude_radians))
    longitude = dec2dms(math.degrees(longitude_radians))
    return latitude, longitude, h


if __name__ == "__main__":
    # Alice Springs GDA94 (ALIC)
    # Example 3.1.1 pg 26 Geocentric Datum of Australia 2020 Technical Manual
    # Version 1.2
    ...
    proj = Projection(a, invf, m0, false_easting, false_northing)
    ...
    x, y, z = geodetic_to_cartesian(latitude, longitude, h, proj)
    ...
    lat_calc, lon_calc, h_calc = cartesian_to_geodetic(x, y, z, proj)
    ...
    calc_east, calc_north, psf, grid_conv = geo_to_grid(lat, lon, cm, proj)
    print(psf, grid_conv)
    print("calc east: {0} diff: {1:.5f}".format(calc_east, calc_east-east))
    print("calc north: {0} diff: {1:.5f}".format(calc_north, calc_north-north))

    # convert from mga2020 to lat/long
    calc_lat, calc_lon = grid_to_geo(calc_east, calc_north, cm, proj)

    print("calc latitude: {0} diff: {1:.5f}".format(calc_lat, calc_lat-lat))
    print("calc longitude: {0} diff: {1:.5f}".format(calc_lon, calc_lon-lon))
